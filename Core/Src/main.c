...
void hx711_tare(void)
{
    ...
    {
        // Wait up to 2 seconds during tare only
...
float hx711_get_grams(void)
{
    ...
    if (raw == 0) return thrust_g; // return last good value if timeout
...
int main(void)
{
  ...
  HAL_Delay(500);
  hx711_tare();
  HAL_Delay(2000); // allow ESC to arm at 1000us
  sweep_start = HAL_GetTick();
  ...
  while (1)
  {
	...
	static float last_good_thrust = 0.0f;
	if (thrust_g > 2000.0f) {
	    thrust_g = last_good_thrust;
	} else {
	    last_good_thrust = thrust_g;
	}
    ...
    uint32_t elapsed = HAL_GetTick() - sweep_start;

    // 0-15s: ramp up
    if (elapsed < 15000) {
        pulse_width = 1000 + (int)((elapsed / 15000.0f) * 800);
        if (pulse_width > 1800) pulse_width = 1800;
    }
    // 15-17s: hold
    ...
    // 17-32s: ramp down
        ...
        pulse_width = 1800 - (int)(((elapsed - 17000) / 15000.0f) * 800);
